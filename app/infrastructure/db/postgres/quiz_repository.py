...
from app.infrastructure.db.mappers import (quiz_entity_to_model,
                                           quiz_model_to_entity)
...
class PostgresQuizRepository(IQuizRepository):
    ...
    async def count_by_user(self, user_id: uuid.UUID) -> int:
        result = await self._session.execute(
            select(func.count()).select_from(QuizModel).where(QuizModel.user_id == user_id)
        )

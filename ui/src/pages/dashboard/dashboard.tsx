import { dashboardApi, challengesApi } from "@/lib/api";
...
export default function DashboardPage() {
  ...
  const pendingChallenges = challenges?.filter((c) => c.status === "pending").slice(0, 3) ?? [];
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card featured className="relative overflow-hidden border-none text-white shadow-lg shadow-primary-500/20">
          <div className="relative z-10 flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Flame className="h-5 w-5 text-accent-300 fill-accent-300" />
                <span className="text-sm font-bold uppercase tracking-wider text-primary-200">Study Streak</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {mostRecentDoc ? (
            <Card className="group cursor-pointer hover:border-primary-200 transition-all" onClick={() => navigate(`/study/${mostRecentDoc.id}`)}>
              <CardContent className="p-6 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-neutral-500 text-sm mt-1 mb-3">
                    Last accessed {new Date(mostRecentDoc.created_at).toLocaleDateString()}
                  </p>
                ...
                <Button size="lg" className="rounded-full h-12 w-12 p-0 flex-shrink-0 shadow-lg shadow-primary-500/20 group-hover:scale-105 transition-transform">
            ...
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-10">
                <p className="text-neutral-500 mb-4">No recent documents found.</p>
          ...
          {/* Study Time Graph */}
          <div className="pt-2"> {/* Visual separation */}
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <h3 className="font-display text-sm font-semibold text-neutral-900 uppercase tracking-wider">Quick Actions</h3>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-neutral-100">
                ...
                <button
                  className="w-full flex items-center gap-3 p-4 hover:bg-neutral-50 transition-colors text-left"
                // onClick={() => navigate("/quiz")} // TODO: Link to quiz creation
                >
                  <div className="h-8 w-8 rounded-lg bg-accent-50 flex items-center justify-center text-accent-600">
                    <Zap className="h-4 w-4" />
                  </div>
                  <span className="text-sm font-medium text-neutral-700">Start Quick Quiz</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-sm font-medium text-neutral-700">Challenge a Friend</span>
                </button>
              </div>
            </CardContent>
          </Card>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-display text-lg font-semibold text-primary-900">Challenges</h2>
            ...
            {pendingChallenges.length > 0 ? (
                ...
                {pendingChallenges.map(c => (
                  <Card key={c.id} className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-bold text-neutral-900">{c.wager_amount} pts</div>
                        <div className="text-xs text-neutral-500">vs Opponent</div>
                      </div>
                    </div>
                    <Button size="sm" variant="outline" onClick={() => navigate("/challenges")}>View</Button>
                  </Card>
                ))}
              </div>
            ) : (
              <Card className="bg-neutral-50 border-dashed">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-neutral-500">No active challenges.</p>
                  <Button variant="ghost" size="sm" onClick={() => navigate("/challenges")}>Create one?</Button>
                </CardContent>
              </Card>
            )}
          </div>
